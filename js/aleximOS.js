...
const AleXimOS = (() => {
  ...
  const _apps = {
    ...
    // ── Fix #5 — Mi Dispositivo (monitor + inventario + calor + VPN) ───────
    mipc: {
  ...
  function _createWindow(appId) {
    ...
    el.id        = winId;
    el.style.cssText = `z-index:${++_zBase}`;

    el.innerHTML = `
      <div class="win-titlebar" id="tb-${winId}">
        <div class="win-controls">
          <button class="win-btn win-close" title="Volver" onclick="AleXimOS.closeWindow('${winId}')"></button>
  ...
  function _makeDraggable(winEl, handle) {
    // Mobile: windows are full-screen panels, no dragging needed
    // Touch on titlebar does nothing (close button handles navigation)
  }
    ...
    overlay.innerHTML = `
      <div class="cronica-article-modal">
        ...
        <div class="cronica-article-body">
          ...
          <p class="cronica-article-text" style="color:var(--text-dim);font-style:italic;">Esta nota fue generada dinámicamente por actividad en la red. Los eventos son ficticios y forman parte del universo narrativo de AleXim Mobile.</p>
        </div>
      </div>`;
      ...
      <div class="tools-content">
        ...
        <div id="panel-about" style="display:none">
          <div style="padding:20px;text-align:center;">
            ...
            <div style="font-family:var(--font-hud);font-size:28px;color:var(--cyan);letter-spacing:4px;margin-bottom:8px;">AleXim Mobile</div>
            <div style="color:var(--text-muted);font-size:12px;">v2.4.1 — Android 14 · AleX-Kernel</div>
            <div style="color:var(--text-dim);font-size:11px;margin-top:12px;">Sistema operativo móvil ficticio para simulación de hacking narrativo.</div>
  ...
  return {
    ...
    closeWindow(winId) {
      const win = _windows[winId];
      if (!win) return;
      // Mobile: slide out to the right before removing
      win.el.style.transition = 'transform 0.2s cubic-bezier(0.55, 0, 1, 0.45), opacity 0.2s ease';
      win.el.style.transform = 'translateX(100%)';
      win.el.style.opacity = '0.4';
      setTimeout(() => {
        win.el.remove();
